...
"""
    ...
    At the start of graph execution, all nodes begin in an inactive state. A node becomes active when it receives a new message (state) on any of its incoming edges (or "channels"). The active node then runs its function and responds with updates. 
    At the end of each super-step, nodes with no incoming messages vote to halt by marking themselves as inactive. The graph execution terminates when all nodes are inactive and no messages are in transit.
"""
...
'''
    ...
    Compiling is a pretty simple step. It provides a few basic checks on the structure of your graph (no orphaned nodes, etc). It is also where you can specify runtime args like checkpointers and breakpoints. 
...
'''
    ...
    Working with Messages in Graph State --> Why use messages? ====================
    Why use messages?¶
    ...
    Working with Messages in Graph State --> Using Messages in your Graph ====================
    ...
    Working with Messages in Graph State --> Serialization ====================
    Serialization¶
    ...
    Working with Messages in Graph State --> MessagesState ====================
...
Edges define how the logic is routed and how the graph decides to stop. This is a big part of how your agents work and how different nodes communicate with each other. There are a few key types of edges:
   ...
   Tip: Use Command instead of conditional edges if you want to combine state updates and routing in a single function.
...
"""
   === Command ====================
    ...
    Important:
      When returning Command in your node functions, you must add return type annotations with the list of node names the node is routing to, e.g. 
      Command[Literal["my_other_node"]]. This is necessary for the graph rendering and tells LangGraph that my_node can navigate to my_other_node.

    Check out this how-to guide for an end-to-end example of how to use Command.


    === Command --> When should I use Command instead of conditional edges? ====================
    ...
    === Command --> Navigating to a node in a parent graph ====================
...
"""
   ...
   @dataclass
    ...
    === Runtime Context --> Recursion Limit ====================
    The recursion limit sets the maximum number of super-steps the graph can execute during a single execution. 
    Once the limit is reached, LangGraph will raise GraphRecursionError. By default this value is set to 25 steps. 
    The recursion limit can be set on any graph at runtime, and is passed to .invoke/.stream via the config dictionary. 
    Importantly, recursion_limit is a standalone config key and should not be passed inside the configurable key as all other user-defined configuration. 
